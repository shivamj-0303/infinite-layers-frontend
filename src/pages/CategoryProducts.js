  ...
  const fetchProducts = async (page) => {
    try {
      ...
      if (categoryProducts.length > 0) {
        setCategory(categoryProducts[0].category);
      }
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="bg-gradient-to-r from-pink-500 to-orange-500 text-white py-8 md:py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <button onClick={() => navigate('/')} className="text-white hover:text-gray-200 mb-4 text-sm">
            ← Back to Home
          </button>
          <h1 className="text-3xl md:text-4xl font-bold mb-2">{category?.name || 'Products'}</h1>
          <p className="text-lg opacity-90">Explore our collection of {category?.name}</p>
        </div>
      </section>
